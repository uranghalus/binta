...
export default function JabatanToolbar({ table }: Props) {
    ...
    const handleBulkDelete = () => {
        ...
        router.post(
            route('jabatan.bulk-delete'),
            { ids: selectedIds },
            {
                onSuccess: () => {
                    toast.success('Success', { description: `${selectedCount} Jabatan berhasil dihapus.` });
                    table.resetRowSelection();
                },
                onError: () => {
                    toast.error('Error!', { description: 'Gagal menghapus Jabatan. Coba lagi.' });
                },
                onFinish: () => {
        ...
        <div className="flex items-center justify-between">
            <div className="flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2">
                <Input
                    ...
                    value={(table.getColumn('nama_jabatan')?.getFilterValue() as string) ?? ''}
