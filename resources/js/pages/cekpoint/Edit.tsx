// resources/js/Pages/cp-inspection/Edit.tsx
...
export default function Edit({ inspection, cekPoints }: Props) {
    const [openCP, setOpenCP] = useState(false);
    const [openModal, setOpenModal] = useState<string | null>(null);

    const { data, setData, put, processing, errors } = useForm({
        kode_cp: inspection.kode_cp ?? "",
        regu: inspection.regu ?? "PAGI",
        kondisi: inspection.kondisi ?? "",
        foto_kondisi: inspection.foto_kondisi_url ?? "",
        bocoran: inspection.bocoran ?? "",
        foto_bocoran: inspection.foto_bocoran_url ?? "",
        penerangan_lampu: inspection.penerangan_lampu ?? "",
        foto_penerangan_lampu: inspection.foto_penerangan_lampu_url ?? "",
        kerusakan_fasum: inspection.kerusakan_fasum ?? "",
        foto_kerusakan_fasum: inspection.foto_kerusakan_fasum_url ?? "",
        potensi_bahaya_api: inspection.potensi_bahaya_api ?? "",
        foto_potensi_bahaya_api: inspection.foto_potensi_bahaya_api_url ?? "",
        potensi_bahaya_keorang: inspection.potensi_bahaya_keorang ?? "",
        foto_potensi_bahaya_keorang: inspection.foto_potensi_bahaya_keorang_url ?? "",
        orang_mencurigakan: inspection.orang_mencurigakan ?? "",
        foto_orang_mencurigakan: inspection.foto_orang_mencurigakan_url ?? "",
        tanggal_patroli: inspection.tanggal_patroli ?? new Date().toISOString().slice(0, 10),
    });
    ...
    const onSubmit = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        put(route('inspection.cp-security.update', inspection.id), {
            onSuccess: () => {
                toast.success('Data berhasil diperbarui!', { description: 'Data CP Inspection berhasil diperbarui.' });
            },
            onError: () => {
                toast.error('Gagal memperbarui data!', { description: 'Data CP Inspection gagal disimpan.' });
                console.log(errors);
            },
            preserveScroll: true,
        });
    ...
    return (
        <AppLayout title="Edit Inspeksi Cek Point">
            <Head title="Edit Inspeksi Cek Point" />
            <Card>
                <CardHeader>
                    <CardTitle>Edit Inspeksi Cek Point</CardTitle>
                    <CardDescription className="text-muted-foreground">
                        Ubah data inspeksi cek point security
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <form onSubmit={onSubmit} className="grid grid-cols-1 gap-4 sm:grid-cols-2" id="editCpForm">
                        <div className="space-y-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Select
                                    value={data.regu}
                                    onValueChange={(value) => setData('regu', value as 'PAGI' | 'SIANG' | 'MALAM' | 'MIDDLE')}
                                >
                ...
                <CardFooter className="flex items-center justify-between">
                    ...
                    <Button type="submit" form="editCpForm" disabled={processing}>
